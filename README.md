...
- ✏️ **直觀編輯** - 清晰顯示 AI 建議，支援複製貼上修改，保留個人風格
- ⌨️ **鍵盤快捷鍵** - 支援 Ctrl+A/E/U/K 等終端編輯快捷鍵
- ✅ **格式驗證** - 自動檢查 commit 訊息是否符合 Conventional Commits 規範
- 🎯 **繁體中文支援** - 生成的訊息使用繁體中文描述
- 🚀 **全域命令** - 安裝後可在任何 Git 專案中直接使用
- 📦 **零配置** - 只需設定 API 金鑰即可開始使用
- 🧠 **智慧截斷** - 大型 diff 自動優先保留實際變更行（`+`/`-`），確保 AI 獲得完整的修改內容，避免分析不完整
...
──────────────────────────────────────────────────────────────────────
💡 提示:
   • 可以複製上面的建議訊息，修改後貼到下方輸入框
   • 或直接輸入全新的 commit 訊息
   • 快捷鍵: Ctrl+A=行首 | Ctrl+E=行尾 | Ctrl+U=清空
──────────────────────────────────────────────────────────────────────

請輸入 commit 訊息 > feat: 新增使用者登入與註冊功能
...
- **`y`** - 直接使用 AI 生成的訊息提交
- **`e`** - 編輯訊息後再提交
  - 會顯示清晰的編輯界面，包含 AI 建議訊息
  - 可以複製 AI 建議並修改，或完全重新輸入
  - 支援鍵盤快捷鍵輔助編輯
- **`n`** - 取消提交

### 編輯模式快捷鍵

在編輯 commit 訊息時，支援以下終端快捷鍵：
